...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        {
            Ray ray = { player.camera.position, Vector3Subtract(player.camera.target, player.camera.position) };
            ...
            for (int i = 0; i < blockCount; i++)
            {
                if (blocks[i].active)
                {
                    BoundingBox bb = { Vector3SubtractValue(blocks[i].position, 0.5f), Vector3AddValue(blocks[i].position, 0.5f) };
            ...
            if (closestBlock != -1)
            {
                if (IsMouseButtonPressed(MOUSE_RIGHT_BUTTON))
                {
                    ...
                    {
                        blocks[blockCount].position = Vector3Add(blocks[closestBlock].position, closestCollision.normal);
                        blocks[blockCount].active = true;
                        blockCount++;
                    }
                }
